...
namespace Framework.Infrastructure
{
    public class Log : ILog
    {
        ...
        public Log(IBaseConfiguration config)
        {
            ...
            debugTarget.Layout = "${longdate}\t${event-context:item=severity}\t${message}";
        ...
        public void Trace(string str, [CallerLineNumber] int sourceLineNumber = 0, [CallerMemberName] string memberName = "", [CallerFilePath] string sourceFilePath = "")
        {
            if (config.LogTraceEnable)
                LogEvent("TRACE", LogLevel.Trace, str,"","", sourceLineNumber, memberName, sourceFilePath);
        }
        public void Debug(string str, [CallerLineNumber] int sourceLineNumber = 0, [CallerMemberName] string memberName = "", [CallerFilePath] string sourceFilePath = "")
        {
            if (config.LogDebugEnable)
                LogEvent("DEBUG", LogLevel.Debug, str, "", "", sourceLineNumber, memberName, sourceFilePath);
        }
        ...
        public void Info(string str, [CallerLineNumber] int sourceLineNumber = 0, [CallerMemberName] string memberName = "", [CallerFilePath] string sourceFilePath = "")
        {
            if (config.LogInfoEnable)
                LogEvent("INFO", LogLevel.Info, str, "", "", sourceLineNumber, memberName, sourceFilePath);
        }
        ...
        public void Warn(string str, [CallerLineNumber] int sourceLineNumber = 0, [CallerMemberName] string memberName = "", [CallerFilePath] string sourceFilePath = "")
        {
            if (config.LogWarnEnable)
                LogEvent("WARNING", LogLevel.Warn, str, "", "", sourceLineNumber, memberName, sourceFilePath);
        }
        ...
        {
            LogEvent("ERROR", LogLevel.Error, str, "", "", sourceLineNumber, memberName, sourceFilePath);
        }
        ...
        {
            LogEvent("ERROR", LogLevel.Error, $"{str} Exception - {ex.RecursivelyGetExceptionMessage()}", "", "", sourceLineNumber, memberName, sourceFilePath);
        }
        ...
        {
            LogEvent("ERROR", LogLevel.Error, $"Exception - {ex.RecursivelyGetExceptionMessage()}", "", "", sourceLineNumber, memberName, sourceFilePath);
        }
        ...
        {
            LogEvent("FATAL", LogLevel.Fatal, str, "", "", sourceLineNumber, memberName, sourceFilePath);
        }
        ...
        {
            LogEvent("FATAL", LogLevel.Error, $"{str} Exception - {ex.RecursivelyGetExceptionMessage()}", "", "", sourceLineNumber, memberName, sourceFilePath);
        }
        ...
        {
            LogEvent("FATAL", LogLevel.Error, $"Exception - {ex.RecursivelyGetExceptionMessage()}", "", "", sourceLineNumber, memberName, sourceFilePath);
        }
        public void SqlBeginTransaction(int count, bool functionCalled, [CallerLineNumber] int sourceLineNumber = 0, [CallerMemberName] string memberName = "", [CallerFilePath] string sourceFilePath = "")
        {
            if (config.LogSqlEnable)
                LogEvent("SQL-BEGIN-TRANSACTION", LogLevel.Info, (functionCalled ? "Called" : ("Increment Count - " + count.ToString())), "", "", sourceLineNumber, memberName, sourceFilePath);
        }
        ...
        {
            if (config.LogSqlEnable)
                LogEvent("SQL-COMMIT-TRANSACTION", LogLevel.Info, (functionCalled ? "Called" : (" Decrement Count - " + count.ToString())), "", "", sourceLineNumber, memberName, sourceFilePath);
        }
        ...
        {
            if (config.LogSqlEnable)
                LogEvent("SQL", LogLevel.Info, sqlStr, (ts.TotalMilliseconds / 1000).ToString(), sqlResult,  sourceLineNumber, memberName, sourceFilePath);
        }
        ...
        {
            if (config.LogSqlEnable)
                LogEvent("SQL-ERROR", LogLevel.Info, String.Format("{0} SQL - {1}", ExceptionUtils.RecursivelyGetExceptionMessage(ex), sqlStr), "", "", sourceLineNumber, memberName, sourceFilePath);
        }
        ...
        {
            if (config.LogSqlEnable)
                LogEvent("SQL-ERROR", LogLevel.Info, sqlStr, "", "", sourceLineNumber, memberName, sourceFilePath);
        }
    }
}